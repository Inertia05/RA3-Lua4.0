...
local HEES = {
    ...
    shouldEvacuateConservative = function(self, unit)
        return ObjectStatusIsReallyDamaged(unit) or ObjectStatusIsLightlyDamaged(unit)
    end,
...
function CreateEES(evac_table, evac_command, unit_type)
  ...
  local IEES = {
    ...
    shouldEvacuateConservative = function(self, unit)
        return ObjectStatusIsDamaged(unit)
    end,

    shouldEvacuateAggressive = function(self, unit)
        return ObjectStatusIsLightlyDamaged(unit)
    end,
    ...
    --- Retrieve the evacuation status of a unit by its unit_id.
    --- @param player_index number The index of the player (1-6)
